...
class AttendanceDateSelector {
    constructor() {
        ...
        this.currentSelectedDate = window.globalActiveAttendanceDate || localStorage.getItem('globalActiveAttendanceDate') || "8th";
        this.init();
    }
                    ...
                    <div class="space-y-4 mt-8">
                        ...
                        <div class="date-options-grid">
                            ${this.dateOptions.map(option => `
                                <button type="button" 
                                        class="date-option-btn ${this.currentSelectedDate === option.value ? 'selected' : ''}" 
                        ...
                        <div class="current-selection mt-6 p-4 bg-gray-700 rounded-lg">
                            <p class="text-white text-center">
                                ...
                                <span id="currentDateDisplay">${this.currentSelectedDate}</span>
                            </p>
                        </div>
    ...
    async applySelection() {
        try {
            ...
            localStorage.setItem('globalActiveAttendanceDate', this.currentSelectedDate);

            // Save to database for persistence across sessions
            if (window.supabase && window.CURRENT_TABLE) {
                await window.supabase
                    .from(window.CURRENT_TABLE)
                    .upsert({
            ...
            if (window.showToast) {
                window.showToast('success', `Attendance date set to ${this.currentSelectedDate}`);
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Set initial date to 8th if no date is currently selected
    if (!window.globalActiveAttendanceDate) {
        window.attendanceDateSelector.setDate("8th");
    }
});
...
window.getCurrentAttendanceDate = function() {
    ...
    return window.globalActiveAttendanceDate || "8th";
};